...
RUN <<-EOF
    ...
    xx-verify /aerospike-backup-cli/target/asbackup_${TARGETOS}_${TARGETARCH}
    xx-verify /aerospike-backup-cli/target/asrestore_${TARGETOS}_${TARGETARCH}
EOF
...
COPY --chown=abuser:abgroup --chmod=0755 --from=builder \
    /aerospike-backup-cli/target/asrestore_${TARGETOS}_${TARGETARCH} \
    /usr/bin/asrestore

COPY --chown=abuser:abgroup --chmod=0755 --from=builder \
    /aerospike-backup-cli/target/asbackup_${TARGETOS}_${TARGETARCH} \
    /usr/bin/asbackup
