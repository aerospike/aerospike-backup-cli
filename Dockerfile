...
RUN --mount=type=secret,id=GOPROXY <<-EOF
    ...
    xx-verify /app/aerospike-backup-cli/dist/asbackup_${TARGETOS}_${TARGETARCH}
    xx-verify /app/aerospike-backup-cli/dist/asrestore_${TARGETOS}_${TARGETARCH}
EOF
...
COPY --chown=abtuser:abtgroup --chmod=0755 --from=builder \
    /app/aerospike-backup-cli/dist/asrestore_${TARGETOS}_${TARGETARCH} \
    /usr/bin/asrestore

COPY --chown=abtuser:abtgroup --chmod=0755 --from=builder \
    /app/aerospike-backup-cli/dist/asbackup_${TARGETOS}_${TARGETARCH} \
    /usr/bin/asbackup
